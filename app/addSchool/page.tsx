...
export default function AddSchoolForm() {
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const res = await fetch("/showSchools", {
        method: "POST",
        body: formData,
      });
      ...
      <div className="flex flex-col md:col-span-2">
        ...
        <input
          ...
          {...register("image", {
            ...
            validate: {
              size: (files) => (files && files.length > 0 && files[0].size <= 5 * 1024 * 1024) || "Max file size 5MB",
              type: (files) => (files && files.length > 0 && ["image/jpeg", "image/png", "image/webp"].includes(files[0].type)) || "Only JPEG/PNG/WEBP"
            }
          })}
        />
        {imageFile && <p className="mt-1 text-xs text-gray-500">{imageFile.name} • {(imageFile.size / 1024).toFixed(0)} KB</p>}
